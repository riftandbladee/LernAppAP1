...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Funktion zum Abrufen der ausgewählten Themen
    function getSelectedThemes() {
        const checkboxes = document.querySelectorAll('#theme-selection input[type="checkbox"]:checked');
    ...
    // Initialize the quiz
    function initQuiz(mode = 'full', selectedThemes = null) {
        currentQuestionIndex = 0;
        score = 0;
        let availableQuestions = [];

        if (mode === 'full') {
            availableQuestions = config.questions.filter(q => selectedThemes.includes(q.category));
            shuffledQuestions = shuffleAndSelectQuestions(availableQuestions, QUESTIONS_PER_ROUND);
        } else if (mode === 'wrong') {
            availableQuestions = userStats.wrongQuestions.filter(q => selectedThemes.includes(q.category));
            shuffledQuestions = shuffleAndSelectQuestions(availableQuestions, QUESTIONS_PER_ROUND);
            if (shuffledQuestions.length === 0) {
                questionElement.textContent = "Keine falschen Fragen zum Üben in den ausgewählten Themen!";
        ...
        if (shuffledQuestions.length === 0) {
            questionElement.textContent = "Keine Fragen für die ausgewählten Themen verfügbar!";
            answersContainer.innerHTML = '';
            checkButton.style.display = "none";
            backToMenuFromQuiz.style.display = "block";
            return;
    ...
    function finishQuiz() {
        ...
        document.getElementById('practice-wrong-button').addEventListener('click', () => {
            const selectedThemes = getSelectedThemes();
            if (selectedThemes.length === 0) {
                alert("Bitte wähle mindestens ein Thema aus!");
                return;
            }
            initQuiz('wrong', selectedThemes);
        });
